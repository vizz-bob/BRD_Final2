...
export default function ThirdPartyUsers() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setValidationError(err.response?.data ? Object.values(err.response.data).flat().join(" ") : "Failed to create user");
    }
  };
  ...
  const handleUpdate = async () => {
    try {
      ...
      setValidationError(err.response?.data ? Object.values(err.response.data).flat().join(" ") : "Update failed");
    }
  };
  ...
  const renderFormFields = (data, setData) => (
    <>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6">
        ...
        <FormGroup label="Pincode"><InputBox value={data.pincode} onChange={e => setData({ ...data, pincode: e.target.value })} placeholder="000000" /></FormGroup>
      </div>
    </>
  );
