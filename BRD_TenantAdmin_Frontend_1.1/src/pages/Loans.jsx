...
import { loanAPI as productLoanAPI } from "../services/loanService";
import { masterProductService } from "../services/masterApiService"; // ← NEW

// =============================================================================
// SECTION 1: GLOBAL CONFIGURATION & CONSTANTS
// =============================================================================

// REMOVED: Static LOAN_TYPES array.
// Loan types are now fetched dynamically from the master backend (port 8001).
// The master's Product.product_name field is used as both label and value.
          ...
          <div className="pt-4 border-t border-slate-100">
            <FormGroup label="Accrual Method">
              <SelectBox
                ...
                options={[
                  { label: "Simple Interest (Flat)", value: "SIMPLE" },
                  { label: "Compound (Reducing)", value: "COMPOUND" },
                ]}
              />
            </FormGroup>
          </div>
        ...
        <div className="lg:col-span-1 space-y-6 sm:space-y-8">
          <FormGroup label="Schedule Type">
            <SelectBox
              ...
              options={[
                { label: "Amortized EMI", value: "EMI" },
                { label: "Bullet / Balloon Payment", value: "BULLET" },
                { label: "Step-Up Repayment", value: "STEP_UP" },
              ]}
            />
          </FormGroup>
      ...
      <div className="p-4 sm:p-6 lg:p-10 grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-12">
        <div className="space-y-6 sm:space-y-8">
          <div>
            ...
            <div className="grid grid-cols-2 gap-4 sm:gap-6">
              <FormGroup label="Min Age">
                <InputBox type="number" value={form.min_age} onChange={(e) => setForm({ ...form, min_age: e.target.value })} suffix="YRS" />
              </FormGroup>
              <FormGroup label="Max Age">
                <InputBox type="number" value={form.max_age} onChange={(e) => setForm({ ...form, max_age: e.target.value })} suffix="YRS" />
              </FormGroup>
            </div>
          </div>
        ...
        <div className="space-y-6 sm:space-y-8">
            ...
            <div className="bg-emerald-50/30 p-4 sm:p-6 rounded-2xl border border-emerald-100/50">
              <FormGroup label="Minimum Health Card Score" helper="Threshold for automated rejection.">
                <InputBox
                  ...
                  onChange={(e) => setForm({ ...form, min_cibil_score: e.target.value })}
                  prefix="🫀"
                  placeholder="e.g. 700"
                />
              </FormGroup>
              <div className="mt-3 flex items-center gap-2 relative">
                ...
                <div className="flex justify-between w-full absolute top-3 text-[9px] font-bold text-slate-400">
          ...
          <FormGroup label="Internal Policy Name">
            <InputBox value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} />
          </FormGroup>
        </div>
      </div>
...
const FeesManager = ({ product, existingCharges, onSave }) => {
  ...
  const [newFee, setNewFee] = useState({
    name: "", category: "FEE", frequency: "ONE_TIME", basis: "FIXED", value: "", recovery_stage: "DISBURSEMENT",
  });
    ...
    <div className="space-y-6 sm:space-y-8">
        ...
        {charges.map((fee, index) => (
          <div key={fee.id || index} className="bg-white p-4 sm:p-6 rounded-2xl border border-slate-200 flex flex-wrap sm:flex-nowrap justify-between items-center gap-4 shadow-sm hover:shadow-lg transition-all duration-300 group">
            ...
            <div className="flex items-center gap-4 sm:gap-10 ml-auto">
              ...
              <button onClick={() => handleRemoveFee(fee.id)} className="w-10 h-10 sm:w-12 sm:h-12 flex items-center justify-center rounded-2xl bg-slate-50 text-slate-400 hover:bg-rose-50 hover:text-rose-500 transition-all border border-slate-200 shadow-sm shrink-0">
                <Icons.Trash />
              </button>
            </div>
          </div>
      ...
      {isCreating ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 sm:gap-8 mb-8 sm:mb-10">
              ...
              <FormGroup label="Category">
                <SelectBox
                  ...
                  options={[
                    { label: "Standard Fee", value: "FEE" },
                    { label: "Penalty / Charge", value: "PENALTY" },
                  ]}
                />
              </FormGroup>
              <div className="flex flex-col gap-2 mb-4">
                ...
                <div className="flex shadow-sm rounded-xl overflow-hidden border border-slate-300 focus-within:ring-[3px] focus-within:ring-indigo-500/20 focus-within:border-indigo-600 transition-all">
                  <input
                    type="number"
                    className="w-full p-3.5 font-bold text-slate-800 outline-none min-w-0"
                    placeholder="0.00"
                    value={newFee.value}
                    onChange={(e) => setNewFee({ ...newFee, value: e.target.value })}
                  />
                  <select
                    className="bg-slate-50 border-l border-slate-300 px-2 sm:px-4 font-bold text-slate-600 text-sm outline-none hover:bg-slate-100 cursor-pointer shrink-0"
                    value={newFee.basis}
                    onChange={(e) => setNewFee({ ...newFee, basis: e.target.value })}
                  >
        ...
        <button
          ...
          className="w-full py-6 sm:py-8 border-2 border-dashed border-slate-300 text-slate-400 font-bold rounded-3xl hover:border-emerald-500 hover:text-emerald-600 hover:bg-emerald-50/50 transition-all text-xs sm:text-sm uppercase tracking-widest flex flex-col items-center gap-2 sm:gap-3 group"
        >
...
// ─── loanTypes prop is passed down from Loans (fetched from master backend) ───
      ...
      <div className="p-4 sm:p-8 lg:p-10 space-y-8 sm:space-y-12">
        <div className="space-y-6 sm:space-y-8">
          ...
          <div className="grid grid-cols-1 gap-6 sm:gap-8 pl-2 sm:pl-4">
            ...
            {/* ── Loan Type: dynamically populated from master backend ── */}
            <FormGroup label="Loan Type">
              {loanTypes.length === 0 ? (
                <div className="text-xs text-slate-400 italic p-3 border border-slate-200 rounded-xl bg-slate-50">
                  Loading loan types from master...
                </div>
              ) : (
                <SelectBox
        ...
        <div className="space-y-6 sm:space-y-8">
          ...
          <div className="grid grid-cols-2 gap-4 sm:gap-8 pl-2 sm:pl-4">
            <FormGroup label="Min Amount">
              <InputBox type="number" prefix="₹" value={form.min_amount} onChange={(e) => setForm({ ...form, min_amount: e.target.value })} />
            </FormGroup>
            <FormGroup label="Max Amount">
              <InputBox type="number" prefix="₹" value={form.max_amount} onChange={(e) => setForm({ ...form, max_amount: e.target.value })} />
            </FormGroup>
            <FormGroup label="Min Tenure">
              <InputBox type="number" suffix="MONTHS" value={form.min_tenure} onChange={(e) => setForm({ ...form, min_tenure: e.target.value })} />
            </FormGroup>
            <FormGroup label="Max Tenure">
              <InputBox type="number" suffix="MONTHS" value={form.max_tenure} onChange={(e) => setForm({ ...form, max_tenure: e.target.value })} />
            </FormGroup>
          </div>
        </div>
      </div>
      ...
      <div className="bg-slate-900 text-white p-5 sm:p-8 flex flex-wrap justify-between items-center gap-4 relative overflow-hidden">
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-3 sm:gap-6">
            ...
            <div>
              <h1 className="text-xl sm:text-3xl font-bold tracking-tight">{product.name}</h1>
...
// ── ProductManager receives loanTypes and forwards to EditProductForm ──
  ...
  return (
    <div className="bg-white rounded-3xl shadow-xl border border-slate-200 overflow-hidden min-h-[85vh] flex flex-col">
      <div className="bg-slate-900 text-white p-4 sm:p-8 flex justify-between items-start sm:items-center gap-3">
        <div className="min-w-0">
          <div className="flex items-center gap-3 sm:gap-4 flex-wrap">
            ...
            <h1 className="text-lg sm:text-2xl font-bold truncate">{currentProduct.name}</h1>
          </div>
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="lg:hidden bg-slate-800 text-slate-300 hover:text-white px-3 py-2 rounded-lg text-xs font-bold uppercase tracking-wide transition"
          >
            ☰ Menu
          </button>
      ...
      <div className="flex flex-1 relative">
        ...
        <div className={`
          absolute lg:static top-0 left-0 h-full z-30 lg:z-auto
          w-72 lg:w-64 xl:w-80 bg-slate-50 border-r border-slate-200 p-4 sm:p-6 space-y-2 sm:space-y-3
        ...
        `}>
          {tabs.map((tab) => (
            ...
            >
              <span className="text-lg sm:text-xl opacity-80 shrink-0">{tab.icon}</span>
              {tab.label}
            </button>
          ))}
        </div>
          ...
          {loading ? (
            ...
            <div className="max-w-6xl mx-auto">
              {activeTab === "overview" && (
                <>
                  {isEditing ? (
                    // ── Pass loanTypes down so EditProductForm can use master's products ──
                    <EditProductForm
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 sm:gap-10">
                      ...
                      <div className="bg-white border border-slate-200 rounded-3xl p-5 sm:p-8 shadow-sm">
                        <div className="flex items-center gap-3 mb-4">
                          <Icons.Health />
                          <span className="font-bold text-rose-800 uppercase tracking-wider text-xs">Health Card Score</span>
                        </div>
                        ...
                        <div className="flex justify-between text-[10px] font-bold text-slate-400 uppercase tracking-wider">
                          <span>Poor (300)</span><span>Excellent (900)</span>
                        </div>
...
// ── loanTypes prop received from Loans (fetched from master backend) ──
const CreateProductForm = ({ onCancel, onSuccess, loanTypes }) => {
  const [form, setForm] = useState({
    name: "", loan_type: "", min_amount: "", max_amount: "", min_tenure: "", max_tenure: "", is_active: true,
  });

  const handleSubmit = async () => {
    if (!form.name || !form.min_amount || !form.max_amount || !form.loan_type) {
      alert("Please fill in all required fields.");
      return;
    }
    try {
      await productLoanAPI.create({
        name: form.name,
        loan_type: form.loan_type,
        min_amount: Number(form.min_amount),
        max_amount: Number(form.max_amount),
        min_tenure: Number(form.min_tenure || 0),
        max_tenure: Number(form.max_tenure || 0),
      ...
      <div className="p-4 sm:p-8 lg:p-12 space-y-10 sm:space-y-16">
        <div className="space-y-6 sm:space-y-8">
          ...
          <div className="grid grid-cols-1 gap-6 sm:gap-10 pl-2 sm:pl-4">
            <FormGroup label="Product Name" required>
              <InputBox placeholder="e.g. Salaried Personal Loan 2025" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} />
            </FormGroup>
            {/* ── Loan Type: dynamically populated from master backend ── */}
            <FormGroup label="Loan Type" required>
              {loanTypes.length === 0 ? (
                <div className="text-xs text-slate-400 italic p-3 border border-slate-200 rounded-xl bg-slate-50 animate-pulse">
                  Loading loan types from master...
                </div>
              ) : (
                <SelectBox
        ...
        <div className="space-y-6 sm:space-y-8">
          ...
          <div className="grid grid-cols-2 gap-4 sm:gap-10 pl-2 sm:pl-4">
            <FormGroup label="Min Amount" required>
              <InputBox type="number" prefix="₹" value={form.min_amount} onChange={(e) => setForm({ ...form, min_amount: e.target.value })} />
            </FormGroup>
            <FormGroup label="Max Amount" required>
              <InputBox type="number" prefix="₹" value={form.max_amount} onChange={(e) => setForm({ ...form, max_amount: e.target.value })} />
            </FormGroup>
            <FormGroup label="Min Tenure">
              <InputBox type="number" suffix="MO" value={form.min_tenure} onChange={(e) => setForm({ ...form, min_tenure: e.target.value })} />
            </FormGroup>
            <FormGroup label="Max Tenure">
              <InputBox type="number" suffix="MO" value={form.max_tenure} onChange={(e) => setForm({ ...form, max_tenure: e.target.value })} />
            </FormGroup>
          </div>
        </div>
...
export default function Loans() {
  ...
  // ── Dynamic loan types fetched from master backend (port 8001) ──
  ...
  }, []); // Runs once on mount — master products don't change mid-session
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await productLoanAPI.delete(id);
      loadProducts();
    } catch (e) {
...
const toggleStatus = async (p) => {
  ...
  const pageWrapper = (children) => (
    <div className="p-3 sm:p-6 lg:p-8 bg-slate-50 min-h-screen">{children}</div>
  );

  // ── Pass loanTypes down to all views that need the dropdown ──
  if (view === "create") return pageWrapper(
    <CreateProductForm
  ...
  if (view === "manage" && selectedProduct) return pageWrapper(
    <ProductManager
  ...
  if (view === "details" && selectedProduct) return pageWrapper(
    <ProductDetails
      product={selectedProduct}
      onBack={() => { setSelectedProduct(null); setView("list"); }}
    />
      ...
      <div className="bg-white rounded-2xl sm:rounded-[2rem] shadow-xl border border-slate-200 overflow-hidden">
        ...
        <div className="p-3 sm:p-6 lg:p-8 border-b border-slate-100 bg-white">
          <div className="relative w-full max-w-2xl">
            <span className="absolute left-4 sm:left-5 top-1/2 -translate-y-1/2 text-slate-400">
              <Icons.Search />
            </span>
            <input
          ...
          <table className="w-full text-left" style={{ minWidth: "600px" }}>
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredProducts.map((p) => (
                <tr key={p.id} className="hover:bg-indigo-50/30 transition-colors group">
                  <td className="px-4 sm:px-6 lg:px-10 py-4 sm:py-6 lg:py-8">
                    ...
                    <div className="text-[10px] text-slate-400 font-bold bg-slate-100 inline-block px-2 py-0.5 rounded whitespace-nowrap">
                      {p.min_tenure}-{p.max_tenure} Mo
                    </div>
                  </td>
                  <td className="px-4 sm:px-6 lg:px-10 py-4 sm:py-6 lg:py-8">
                    <Badge color="purple">{p.loan_type}</Badge>
                  </td>
                  ...
                  <td className="px-4 sm:px-6 lg:px-10 py-4 sm:py-6 lg:py-8 text-right">
                    <div className="flex justify-end gap-1 sm:gap-2 opacity-60 group-hover:opacity-100 transition-all">
                      <Button variant="secondary" size="sm" onClick={() => handleView(p)} icon={<Icons.Eye />}>
              ...
              {!filteredProducts.length && (
                <tr>
                  <td colSpan={5} className="px-6 py-12 sm:py-24 text-center">
