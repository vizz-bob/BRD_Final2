...
export default function Users() {
  ...
  const initialForm = {
    tenant_id: currentTenant, first_name: "", last_name: "", email: "",
  ...
  const roleMap = useMemo(() => roles.reduce((acc, role) => { acc[role.id] = role.name; return acc; }, {}), [roles]);
  ...
  const mapFormToPayload = (formData) => {
    const payload = {
      tenant: currentTenant, first_name: formData.first_name, last_name: formData.last_name,
      email: formData.email, mobile_number: formData.mobile_no, role_type: formData.role_type,
      role_id: formData.role_id || null, is_active: formData.status === 'Active'
    };
    if (formData.password) payload.password = formData.password;
    if (isSupervisorRole(formData.role_type)) {
      payload.supervisor_name = formData.supervisor_name;
      payload.supervisor_email = formData.supervisor_email;
      payload.supervisor_mobile = formData.supervisor_mobile;
    }
    return payload;
  };
  ...
  const validateForm = (data, isEdit = false) => {
    ...
    if (!isEdit && !data.password && !isSupervisorRole(data.role_type)) return "Password is required for new users";
    if (isSupervisorRole(data.role_type)) {
      if (!data.supervisor_name?.trim()) return "Supervisor Name is required";
      if (!data.supervisor_email?.trim()) return "Supervisor Email is required";
      if (!emailRegex.test(data.supervisor_email)) return "Invalid Supervisor Email format";
      if (!data.supervisor_mobile?.toString().trim()) return "Supervisor Mobile is required";
    }
    return null;
  };
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      setValidationError(e.response?.data?.detail || "Failed to create user. Please check inputs.");
    }
  };
  ...
  const filteredItems = useMemo(() => users.filter(user => {
    const matchesSearch =
  ...
  const stats = useMemo(() => ({
    ...
    supervisors: users.filter(u => (u.role || u.role_type) === 'SUPERVISOR').length
  }), [users]);
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                ...
                <tr key={u.id} className="hover:bg-indigo-50/30 transition-colors group">
                  ...
                  <td className="px-5 md:px-8 py-4 md:py-5">
                    <span className={`inline-flex items-center px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-wide border whitespace-nowrap ${u.is_active || u.status === 'Active' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-slate-100 text-slate-500 border-slate-200'}`}>
                  ...
                  <td className="px-5 md:px-8 py-4 md:py-5 text-right">
                    <div className="flex justify-end gap-1.5 opacity-60 group-hover:opacity-100 transition-opacity">
                      ...
                      <button onClick={() => { setEditing({...u, mobile_no: u.phone || u.mobile_number, role_type: u.role || u.role_type, status: u.is_active ? "Active" : "Inactive", password: ""}); setValidationError(""); }} className="p-2 text-indigo-600 hover:bg-indigo-50 rounded-lg transition"><Icons.Edit /></button>
            ...
            <div className="p-5 md:p-8 overflow-y-auto space-y-5 md:space-y-8">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6">
                ...
                <FormGroup label="Role ID" required>
                  <SelectBox options={roles.map(r => ({ value: r.id, label: r.name || r.role_name }))} value={form.role_id} onChange={e => setForm({...form, role_id: e.target.value})} placeholder="Select Role ID" icon={<Icons.Key />} />
                </FormGroup>
                {!isSupervisorRole(form.role_type) && (
          ...
          <div className="bg-white w-full max-w-2xl rounded-2xl md:rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div className="p-5 md:p-8 overflow-y-auto space-y-5 md:space-y-8">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6">
                ...
                <FormGroup label="Role ID" required><SelectBox options={roles.map(r => ({ value: r.id, label: r.name || r.role_name }))} value={editing.role_id} onChange={e => setEditing({...editing, role_id: e.target.value})} icon={<Icons.Key />} /></FormGroup>
                {!isSupervisorRole(editing.role_type) && (
          ...
          <div className="bg-white w-full max-w-lg rounded-2xl md:rounded-3xl shadow-2xl p-6 md:p-8 relative max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-3 bg-slate-50 rounded-2xl p-4 md:p-6 border border-slate-100">
              {[
                ...
                { label: "Status", value: viewing.is_active ? "Active" : "Inactive", color: viewing.is_active ? "text-emerald-600" : "text-slate-500" },
