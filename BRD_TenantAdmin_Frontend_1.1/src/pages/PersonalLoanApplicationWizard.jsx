...
export default function PersonalLoanApplicationWizard() {
  ...
  const handleSubmitApplication = async () => {
    ...
    try {
      ...
      if (!Number.isInteger(tenantId) || !Number.isInteger(customerId) || !Number.isInteger(resolvedProductId)) throw new Error("Invalid tenant, customer, or product ID");
            ...
            {currentStep === 1 && (
              <div className="animate-fade-in">
                ...
                <div className="grid grid-cols-2 gap-4 sm:gap-6">
                  ...
                  <SelectGroup name="gender" label="Gender" width="half" value={formData.gender} onChange={handleChange} options={[{ label: "Male", value: "M" }, { label: "Female", value: "F" }, { label: "Other", value: "O" }]} />
              ...
              <div className="animate-fade-in">
                ...
                <div className="bg-blue-50 p-1.5 rounded-xl flex mb-6 sm:mb-8">
                  {["Salaried", "Self-Employed"].map((type) => (
                    <button
                      key={type}
                      onClick={() => setFormData((prev) => ({ ...prev, income_type: type }))}
                      className={`flex-1 py-2 sm:py-2.5 text-xs sm:text-sm font-bold rounded-lg transition-all ${formData.income_type === type ? "bg-white shadow text-blue-700" : "text-blue-400 hover:text-blue-600"}`}
                    >{type}</button>
                  ))}
                </div>
                <div className="grid grid-cols-2 gap-4 sm:gap-6">
                  ...
                  {formData.income_type === "Salaried" ? (
                    <>
                      ...
                      <SelectGroup name="employment_type" label="Employment Type" value={formData.employment_type} onChange={handleChange} options={[{ label: "Private Sector", value: "Private" }, { label: "Public Sector", value: "Public" }, { label: "Government", value: "Govt" }]} />
                    </>
                  ) : (
                    <>
            ...
            {currentStep === 4 && (
              <div className="animate-fade-in">
                ...
                <div className="grid grid-cols-2 gap-4 sm:gap-6">
                  ...
                  <SelectGroup name="account_type" label="Account Type" width="half" value={formData.account_type} onChange={handleChange} options={[{ label: "Savings", value: "Savings" }, { label: "Current", value: "Current" }]} />
