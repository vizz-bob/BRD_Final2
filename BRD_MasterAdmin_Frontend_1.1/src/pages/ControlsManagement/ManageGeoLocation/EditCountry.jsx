...
export default function EditCountry() {
  ...
  useEffect(() => {
    // TODO: fetch country by id
    const fetched = { name: "India", code: "IN" };
    setForm(fetched);
  }, [id]);
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    console.log("Update Country:", form);
    navigate("/controls/geo/country");
  };
    ...
    <MainLayout>
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          ...
          <p className="text-gray-500 text-sm">Update country details</p>
        </div>
      </div>

      <form onSubmit={handleSubmit} className="bg-white p-8 rounded-2xl shadow-md max-w-2xl grid grid-cols-1 md:grid-cols-2 gap-6">
        <Input label="Country Name" name="name" value={form.name} onChange={handleChange} required />
        <Input label="Country Code" name="code" value={form.code} onChange={handleChange} required />

        <div className="md:col-span-2 flex justify-end gap-2 mt-4">
          <button type="button" onClick={() => navigate("/controls/geo/country")} className="px-5 py-3 rounded-xl border text-gray-700">Cancel</button>
