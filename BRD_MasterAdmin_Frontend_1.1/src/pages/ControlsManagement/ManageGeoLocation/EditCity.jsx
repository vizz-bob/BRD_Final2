...
const STATES = ["Maharashtra", "California"];
...
export default function EditCity() {
  ...
  useEffect(() => {
    // TODO: fetch city by id
    setForm({ name: "Mumbai", state: "Maharashtra" });
  }, [id]);
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    console.log("Update City:", form);
    navigate("/controls/geo/cities");
  };
    ...
    <MainLayout>
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          ...
          <p className="text-gray-500 text-sm">Update city details</p>
        </div>
      </div>

      <form onSubmit={handleSubmit} className="bg-white p-8 rounded-2xl shadow-md max-w-2xl grid grid-cols-1 md:grid-cols-2 gap-6">
        <Input label="City Name" name="name" value={form.name} onChange={handleChange} required />
        <Select label="State" name="state" value={form.state} onChange={handleChange} options={STATES} required />

        <div className="md:col-span-2 flex justify-end gap-2 mt-4">
          <button type="button" onClick={() => navigate("/controls/geo/cities")} className="px-5 py-3 rounded-xl border text-gray-700">Cancel</button>
