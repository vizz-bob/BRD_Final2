import React, { useState } from "react";
import MainLayout from "../../../layout/MainLayout";
import { FiArrowLeft, FiSave } from "react-icons/fi";
import { useNavigate } from "react-router-dom";

export default function ClassificationManage() {
  const navigate = useNavigate();

  const [form, setForm] = useState({
    classification_name: "Standard Asset",
    product_type: "",
    status: true
  });

  const PRODUCT_TYPES = ["Personal Loan", "Home Loan", "Business Loan", "Education Loan"];

  const submit = (e) => {
    e.preventDefault();
    console.log(form);
    navigate(-1);
  };

  return (
    <MainLayout>
      {/* HEADER */}
      <div className="flex items-center gap-3 mb-6">
        <button
          onClick={() => navigate(-1)}
          className="p-2 rounded-full bg-gray-100 hover:bg-gray-200"
        >
          <FiArrowLeft size={16} />
        </button>
        <div>
          <h1 className="text-xl font-semibold">Manage Classification</h1>
          <p className="text-sm text-gray-500">
            Enable / disable classification & map to products
          </p>
        </div>
      </div>

      {/* FORM */}
      <form onSubmit={submit} className="bg-white rounded-2xl shadow-sm p-6 max-w-3xl">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">

          {/* Classification Name */}
          <div>
            <label className="text-sm font-medium text-gray-700">
              Classification
            </label>
            <input
              disabled
              value={form.classification_name}
              className="mt-1 w-full rounded-xl border border-gray-300 bg-gray-50 px-3 py-2 text-sm"
            />
          </div>

          {/* Product Type */}
          <div>
            <label className="text-sm font-medium text-gray-700">
              Product Type *
            </label>
            <select
              required
              value={form.product_type}
              onChange={(e) => setForm({ ...form, product_type: e.target.value })}
              className="mt-1 w-full rounded-xl border border-gray-300 px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-blue-500"
            >
              <option value="">Select Product</option>
              {PRODUCT_TYPES.map((p) => (
                <option key={p}>{p}</option>
              ))}
            </select>
          </div>

          {/* Status */}
          <div className="md:col-span-2">
            <label className="text-sm font-medium text-gray-700">
              Status
            </label>
            <div className="flex items-center gap-4 mt-2">
              <span className="text-sm text-gray-500">Inactive</span>
              <input
                type="checkbox"
                checked={form.status}
                onChange={(e) => setForm({ ...form, status: e.target.checked })}
                className="w-12 h-6 accent-blue-600 cursor-pointer"
              />
              <span className="text-sm text-gray-500">Active</span>
            </div>
          </div>
        </div>

        {/* ACTIONS */}
        <div className="flex justify-end gap-3 mt-8">
          <button
            type="button"
            onClick={() => navigate(-1)}
            className="px-5 py-2 rounded-xl bg-gray-100 text-sm hover:bg-gray-200"
          >
            Cancel
          </button>
          <button
            type="submit"
            className="px-5 py-2 rounded-xl bg-blue-600 text-white text-sm flex items-center gap-2 hover:bg-blue-700"
          >
            <FiSave size={16} /> Save Mapping
          </button>
        </div>
      </form>
    </MainLayout>
  );
}
