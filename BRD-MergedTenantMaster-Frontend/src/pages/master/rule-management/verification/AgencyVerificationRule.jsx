    ...
    <>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500">Third-party verification configuration</p>
        </div>
        <button
          onClick={() => navigate("/rule-management/verification/agency/add")}
        ...
        {filtered.map(r => (
          <React.Fragment key={r.id}>
            ...
            <div className="hidden md:grid bg-white rounded-2xl px-5 py-4 shadow-sm grid-cols-5 items-center text-sm">
              <div className="font-medium truncate">{r.agency_type}</div>
              <div>{r.verification_stage}</div>
            ...
            <div className="md:hidden bg-white rounded-2xl shadow-sm divide-y">
              <div className="flex justify-between items-center px-4 py-3">
                <span className="font-semibold text-sm">{r.agency_type}</span>
                <div className="flex gap-3 text-gray-600">
                  <FiEye onClick={() => navigate(`/rule-management/verification/agency/view/${r.id}`)} />
                  <FiEdit3 onClick={() => navigate(`/rule-management/verification/agency/edit/${r.id}`)} />
                  <FiTrash2 onClick={() => setDeleteId(r.id)} />
                </div>
              </div>

              <div className="px-4 py-3 space-y-2 text-sm">
                <Row label="Stage" value={r.verification_stage} />
                <Row label="Report" value={r.report_type} />
