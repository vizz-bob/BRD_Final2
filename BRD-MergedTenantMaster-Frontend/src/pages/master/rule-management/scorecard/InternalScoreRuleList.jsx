  ...
  return (
    <>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
        ...
        <button onClick={() => navigate("/rule-management/scorecard/internal/add")}
      ...
      <div className="bg-white p-4 rounded-2xl mb-6 flex items-center gap-3 shadow-sm">
        ...
        <input value={search} onChange={e => setSearch(e.target.value)}
          placeholder="Search by parameter..." className="w-full outline-none text-sm bg-transparent" />
      </div>
        ...
        {filtered.map(r => (
          <React.Fragment key={r.id}>
            ...
            <div className="hidden md:grid bg-white rounded-2xl px-5 py-4 shadow-sm grid-cols-6 items-center text-sm">
              <div className="font-medium truncate">{r.parameter}</div>
              <div>{r.min_value}</div>
              <div>{r.weight}</div>
            ...
            <div className="md:hidden bg-white rounded-2xl shadow-sm divide-y">
              <div className="flex justify-between items-center px-4 py-3">
                <span className="font-semibold text-sm">{r.parameter}</span>
                <div className="flex gap-3 text-gray-600">
                  <FiEye onClick={() => navigate(`/rule-management/scorecard/internal/view/${r.id}`)} />
                  <FiEdit3 onClick={() => navigate(`/rule-management/scorecard/internal/edit/${r.id}`)} />
                  <FiTrash2 onClick={() => setDeleteId(r.id)} />
                </div>
              </div>

              <div className="px-4 py-3 space-y-2 text-sm">
                <Row label="Min Value" value={r.min_value} />
