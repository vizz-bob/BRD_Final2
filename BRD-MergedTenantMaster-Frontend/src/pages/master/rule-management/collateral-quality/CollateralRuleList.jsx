...
import { DeleteConfirmButton } from "../../../../components/master/Controls/SharedUIHelpers";
    ...
    <>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500">Define acceptance and risk logic for collaterals</p>
        </div>
        <button onClick={() => navigate("/rule-management/collateral-quality/add")}
      ...
      <div className="space-y-4 max-h-[70vh] overflow-y-auto pr-1">
        ...
        <div className="hidden md:grid grid-cols-7 bg-gray-100 rounded-xl px-5 py-3 text-xs font-semibold text-gray-600 sticky top-0 z-10">
          <div>Type</div><div>Ownership</div><div>Min Value</div>
          <div>LTV</div><div>Risk</div><div>Status</div><div className="text-right">Actions</div>
        </div>
        ...
        {filtered.map(r => (
          <React.Fragment key={r.id}>
            ...
            <div className="hidden md:grid bg-white rounded-2xl px-5 py-4 shadow-sm grid-cols-7 items-center text-sm">
              <div className="font-medium truncate">{r.collateral_type}</div>
              <div>{r.ownership}</div>
              ...
              <div className="flex justify-end gap-2">
                <IconButton color="gray" onClick={() => navigate(`/rule-management/collateral-quality/view/${r.id}`)}><FiEye /></IconButton>
                <IconButton color="blue" onClick={() => navigate(`/rule-management/collateral-quality/edit/${r.id}`)}><FiEdit3 /></IconButton>
            ...
            <div className="md:hidden bg-white rounded-2xl shadow-sm divide-y">
              <div className="flex items-center justify-between px-4 py-3">
                <span className="font-semibold text-sm">{r.collateral_type}</span>
                <div className="flex gap-3 text-gray-600">
                  <FiEye onClick={() => navigate(`/rule-management/collateral-quality/view/${r.id}`)} />
                  <FiEdit3 onClick={() => navigate(`/rule-management/collateral-quality/edit/${r.id}`)} />
                  <FiTrash2 onClick={() => setDeleteId(r.id)} />
                </div>
              </div>

              <div className="px-4 py-3 space-y-3 text-sm">
                <Row label="Ownership" value={r.ownership} />
                <Row label="Min Value" value={`₹${r.min_market_value}`} />
                <Row label="Allowed LTV" value={`${r.allowed_ltv}%`} />
                <Row label="Risk Level" value={r.risk_level} />
