...
const RoleList = () => {
  ...
  const handleRemovePermission = async (permId) => {
    if (
      !window.confirm(
        `Remove "${PERMISSION_LABELS[permId] || permId
  ...
  return (
    <>
      ...
      <div className="flex gap-6 relative items-start">
        ...
        <div
          className={`transition-all duration-300 ${selectedRole ? "w-2/3" : "w-full"
            }`}
        >
            ...
            {loading ? (
              ...
              <div className="space-y-4">
                {(Array.isArray(roles) ? roles : []).map((role) => (
                  <div
                    key={role.id}
                    className={`flex items-center justify-between p-5 rounded-xl border transition cursor-pointer ${selectedRole?.id === role.id
            ...
            <div className="p-5 max-h-[60vh] overflow-y-auto">
              {activePermsList.length === 0 ? (
                ...
                <div className="space-y-3">
                  {(Array.isArray(activePermsList) ? activePermsList : []).map((permId) => {
                    // Find the permission object by ID
                    const perm = (Array.isArray(permissions) ? permissions : []).find((p) => p.id === permId) || {};
