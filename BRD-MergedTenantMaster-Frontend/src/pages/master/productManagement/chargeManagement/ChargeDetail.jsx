...
export default function ChargeDetail() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await chargesService.getCharge(id);
        setData(res);
  ...
  if (loading) return <div className="px-8 py-6"><p className="text-gray-500">Loading charge details...</p></div>;
  if (!data) return <div className="px-8 py-6"><p className="text-gray-500">Charge not found.</p></div>;
    ...
    <div className="px-8 py-6">
      <div className="bg-white border border-gray-200 rounded-2xl px-8 py-5 flex items-center justify-between mb-6 shadow-sm max-w-2xl">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-bold text-gray-900">Charge Details</h1>
            <p className="text-sm text-gray-500 mt-0.5">View charge configuration and recovery rules</p>
          </div>
        </div>
        <button onClick={() => navigate(`/charges/edit/${id}`)}
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-8 max-w-2xl space-y-8">
        <Section title="Charge Configuration">
          <DetailItem label="Charge Name" value={data.charge_name} />
          <DetailItem label="Frequency" value={data.frequency} />
          <DetailItem label="Basis of Recovery" value={data.basis_of_recovery} />
          <DetailItem label="Rate of Charges" value={data.rate_of_charges} />
        </Section>
