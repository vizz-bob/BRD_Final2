...
export default function EditCharge() {
  ...
  const [form, setForm] = useState({
    charge_name: "", frequency: "", basis_of_recovery: "",
    recovery_stage: "", recovery_mode: "", rate_of_charges: "",
  });
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setForm({ ...data, rate_of_charges: data.rate_of_charges ?? "" });
  ...
  const handleSubmit = async () => {
    ...
    try {
      await chargesService.updateCharge(id, { ...form, rate_of_charges: Number(form.rate_of_charges) });
      navigate(-1);
    } catch (err) {
  ...
  if (loading) return <div className="px-8 py-6"><p className="text-gray-500">Loading charge...</p></div>;
  ...
  return (
    <div className="px-8 py-6">
      <div className="bg-white border border-gray-200 rounded-2xl px-8 py-5 flex items-center gap-4 mb-6 shadow-sm max-w-2xl">
        ...
        <div>
          <h1 className="text-xl font-bold text-gray-900">Edit Charge</h1>
          <p className="text-sm text-gray-500 mt-0.5">Update charge configuration and recovery rules</p>
        </div>
      </div>
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-8 max-w-2xl space-y-8">
        <Section title="Charge Details">
          <StyledInput label="Charge Name *" value={form.charge_name} placeholder="e.g. Late Payment Charge"
            onChange={e => handleChange("charge_name", e.target.value)} error={fieldErrors.charge_name} />
          ...
          <StyledInput label="Rate of Charges *" type="number" value={form.rate_of_charges} placeholder="e.g. 2.5"
            onChange={e => handleChange("rate_of_charges", e.target.value)} error={fieldErrors.rate_of_charges} />
        </Section>
        ...
        <div className="flex items-center justify-between pt-4 border-t border-gray-100">
          ...
          <button onClick={handleSubmit} disabled={saving}
            ...
            <FiSave size={15} />{saving ? "Updating..." : "Update Charge"}
          </button>
        </div>
      </div>
    </div>
