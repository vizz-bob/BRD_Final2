...
export default function CoApplicantList() {
  ...
  const fetchCoApplicants = async () => {
    ...
    setCoApplicants(Array.isArray(data) ? data : (data?.results || []));
    setLoading(false);
  };
  ...
  const filtered = (Array.isArray(coApplicants) ? coApplicants : []).filter(
    (c) =>
...
const StatusBadge = ({ status }) => (
  <span
    className={`max-w-fit px-3 py-1 text-xs rounded-full ${status === "Active"
