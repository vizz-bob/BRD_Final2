...
export default function TeleVerificationList() {
  ...
  const loadVerifications = async () => {
    setLoading(true);
    const data = await controlsManagementService.verifications.list();
    setVerifications(Array.isArray(data) ? data : (data?.results || []));
    setLoading(false);
  };

  const filtered = (Array.isArray(verifications) ? verifications : []).filter(
    (v) =>
  ...
  const handleDelete = async (id) => {
    ...
    await controlsManagementService.verifications.delete(id);
    loadVerifications();
  };
      ...
      <div className="space-y-3">
        ...
        {filtered.map((v) => (
          ...
          >
            ...
            <span
              className={`px-3 py-1 text-xs rounded-full ${v.status === "Success" ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"
                }`}
            >
              {v.status}
            </span>
