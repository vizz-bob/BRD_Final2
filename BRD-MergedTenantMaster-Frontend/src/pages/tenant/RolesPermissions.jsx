...
import { 
  mapFrontendToDjangoPermissions, 
  mapDjangoToFrontendPermissions, 
  syncRolePermissions 
} from "../../services/permissionMapping.js";
...
const PERMISSION_KEY_MAP = {
  ...
  manage_categories: { category: "system_administration", value: "MANAGE_CATEGORIES" },
  ...
  rule_profile: { category: "rules_management", value: "RULE_PROFILE" },
  rule_collateral: { category: "rules_management", value: "RULE_COLLATERAL" },
  rule_financial: { category: "rules_management", value: "RULE_FINANCIAL" },
  rule_credit: { category: "rules_management", value: "RULE_CREDIT" },
  rule_scorecard: { category: "rules_management", value: "RULE_SCORECARD" },
  rule_geo: { category: "rules_management", value: "RULE_GEO" },
  rule_risk: { category: "rules_management", value: "RULE_RISK" },
  rule_verification: { category: "rules_management", value: "RULE_VERIFICATION" },
};
...
const PERMISSION_DEFINITIONS = {
  ...
  manage_categories: { label: "Manage Categories", desc: "Create and edit category types (Loan, Product, etc.)" },
  ...
  rule_profile: { label: "Rule: Client Profile", desc: "Manage eligibility rules for customer profiles" },
  rule_collateral: { label: "Rule: Collateral", desc: "Manage collateral quality and LTV rules" },
  rule_financial: { label: "Rule: Financial", desc: "Manage income and EMI capability rules" },
  rule_credit: { label: "Rule: Credit Bureau", desc: "Manage credit score and bureau history rules" },
  rule_scorecard: { label: "Rule: Scorecard", desc: "Manage internal score parameters and weights" },
  rule_geo: { label: "Rule: Geo-Location", desc: "Manage risk mapping for states and pincodes" },
  rule_risk: { label: "Rule: Risk Mitigation", desc: "Manage risk actions and severity mappings" },
  rule_verification: { label: "Rule: Verification", desc: "Manage internal and agency verification rules" },
};
...
const PERMISSION_CATEGORIES = [
  ...
  { id: "system_administration", name: "System Administration", icon: <ShieldCheckIcon className="w-4 h-4" />, permissions: ["audit_logs","edit_policies","manage_users","manage_roles","manage_branches","user_management","role_permission","tenant_organization_settings","configuration_masters","workflow_setup","manage_categories"] },
  ...
  { id: "rules_management", name: "Rules Management", icon: <ArrowsUpDownIcon className="w-4 h-4" />, permissions: ["rule_profile","rule_collateral","rule_financial","rule_credit","rule_scorecard","rule_geo","rule_risk","rule_verification"] },
];
...
export default function RolesPermissions() {
  ...
  const loadRoles = async () => {
    ...
    const processed = data.map((r) => {
      ...
      Object.entries(PERMISSION_KEY_MAP).forEach(([uiKey, map]) => {
        // Check both old and new format
        permissions[uiKey] = (r[map.category]?.includes(map.value)) || perms.includes(map.value);
      });
      return { 
        id: r.id, 
        role_id: r.id, 
        ...
        status: r.role_status || r.status || "active", 
        permissions 
      };
    });
    setRoles(processed);
  ...
  const handleEditRole = async (role) => {
    setEditingRole(role);
    try {
      // Get Django permissions for this role
      const permRes = await rolesApi.getPermissions(role.role_id);
      let loadedPerms = {};
      
      if (permRes.ok && permRes.data) {
        // Use permission mapping service to convert Django permissions to frontend
        loadedPerms = mapDjangoToFrontendPermissions(permRes.data);
      }
  ...
  const buildBackendPayload = () => {
    ...
    Object.entries(roleForm.permissions).forEach(([key, enabled]) => {
      ...
      if (map) {
        if (!payload[map.category]) payload[map.category] = [];
        payload[map.category].push(map.value);
      }
    });
    return payload;
  };
  ...
  const handleRoleSubmit = async () => {
    try {
      ...
      console.log('📊 Permissions being saved:', roleForm.permissions);
      
      // Count enabled permissions
      const enabledPermissions = Object.entries(roleForm.permissions).filter(([key, enabled]) => enabled);
      ...
      if (editingRole) {
        ...
        roleResult = await roleAPI.update(editingRole.id, payload);
      } else {
        console.log('➕ Creating new role');
        roleResult = await roleAPI.create(payload);
      }
      
      console.log('✅ Role save result:', roleResult);
      
      // Then, sync Django permissions for the role
      const roleId = editingRole ? editingRole.role_id : roleResult.data.id;
      console.log('🔗 Role ID for permission sync:', roleId);
      
      // Use permission sync service to update Django permissions
      console.log('🔄 Starting permission sync...');
      const syncResult = await syncRolePermissions(roleId, roleForm.permissions);
      
      if (syncResult.ok) {
        showNotification("success", editingRole ? "Role and permissions updated successfully" : "Role and permissions created successfully");
        showNotification("info", "Django admin permissions have been synchronized");
        console.log('🎉 Save process completed successfully!');
      } else {
        showNotification("warning", "Role saved but Django permission sync failed");
        console.log("Sync result details:", syncResult);
      }
      ...
      showNotification("error", "Failed to save role and permissions"); 
    }
  };

  const togglePermission = (key) => { 
    console.log('🔄 Toggling permission:', key);
    console.log('   Current state:', roleForm.permissions[key]);
    console.log('   New state:', !roleForm.permissions[key]);
    ...
    setRoleForm((prev) => { 
      ...
      console.log('✅ Total enabled permissions:', enabledCount);
  ...
  const togglePermissionCategory = (categoryPermissions, shouldEnable) => { 
    console.log('🔄 Toggling category permissions:');
    ...
    setRoleForm((prev) => { 
      ...
      categoryPermissions.forEach((key) => { 
        console.log('   - Setting', key, 'to', shouldEnable);
        newPerms[key] = shouldEnable; 
      });
      
      console.log('📊 Updated permissions state after category toggle:', newPerms);
      
      // Count enabled permissions
      const enabledCount = Object.values(newPerms).filter(Boolean).length;
      console.log('✅ Total enabled permissions:', enabledCount);
  ...
  const handleUserRoleSave = async () => {
    ...
    try {
      ...
      const payload = { 
        ...
        role_type: selectedRoleObj.role_type, 
        account_status: selectedUserForEdit.account_status, 
        first_name: selectedUserForEdit.first_name, 
        last_name: selectedUserForEdit.last_name, 
        email: selectedUserForEdit.email, 
        mobile_number: selectedUserForEdit.mobile_number 
      };
                  ...
                  <div key={user.id} className="bg-white border border-gray-200 rounded-xl p-4 sm:p-5 hover:shadow-md transition-shadow flex flex-col justify-between">
                    <div>
                      <div className="flex items-start justify-between mb-3 sm:mb-4">
                        ...
                        <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wide border shrink-0 ${(user.account_status || "").toUpperCase() === "ACTIVE" ? "bg-green-50 text-green-700 border-green-200" : "bg-red-50 text-red-700 border-red-200"}`}>
