    ...
    <div className="p-4 md:p-8 min-h-screen bg-slate-50 font-sans">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-6">
        <div className="bg-white p-5 md:p-6 rounded-2xl shadow-sm border border-slate-100 flex items-center gap-4">
          ...
          <div><div className="text-2xl font-black text-slate-900">{(Array.isArray(tickets) ? tickets : []).filter(t => t.status === 'OPEN').length}</div><div className="text-xs font-bold text-slate-400 uppercase">Open Tickets</div></div>
        </div>
        <div className="bg-white p-5 md:p-6 rounded-2xl shadow-sm border border-slate-100 flex items-center gap-4">
          <div className="bg-emerald-100 p-3 rounded-xl text-emerald-600 shrink-0"><CheckCircleIcon className="h-6 w-6" /></div>
          <div><div className="text-2xl font-black text-slate-900">{(Array.isArray(tickets) ? tickets : []).filter(t => t.status === 'RESOLVED').length}</div><div className="text-xs font-bold text-slate-400 uppercase">Resolved</div></div>
        </div>
        <div className="bg-white p-5 md:p-6 rounded-2xl shadow-sm border border-slate-100 flex items-center gap-4">
          <div className="bg-blue-100 p-3 rounded-xl text-blue-600 shrink-0"><ChatBubbleLeftRightIcon className="h-6 w-6" /></div>
          <div><div className="text-2xl font-black text-slate-900">{(Array.isArray(tickets) ? tickets : []).length}</div><div className="text-xs font-bold text-slate-400 uppercase">Total Volume</div></div>
        </div>
      </div>
          ...
          <table className="w-full text-left min-w-[600px]">
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? <tr><td colSpan="6" className="p-8 text-center animate-pulse">Loading...</td></tr> :
                (Array.isArray(tickets) ? tickets : []).length === 0 ? <tr><td colSpan="6" className="p-8 text-center text-slate-400">No tickets found.</td></tr> :
                  (Array.isArray(tickets) ? tickets : []).map(t => (
                    <tr key={t.id} className="hover:bg-slate-50 cursor-pointer">
                      ...
                      <td className="px-4 md:px-6 py-4">
                        <span className={`text-[10px] px-2 py-1 rounded font-bold uppercase whitespace-nowrap ${t.priority === 'CRITICAL' ? 'bg-red-100 text-red-700' :
                            t.priority === 'HIGH' ? 'bg-orange-100 text-orange-700' : 'bg-blue-50 text-blue-700'
                          }`}>{t.priority}</span>
                      </td>
                      <td className="px-4 md:px-6 py-4">
                        <span className={`text-[10px] px-2 py-1 rounded font-bold uppercase border whitespace-nowrap ${t.status === 'OPEN' ? 'bg-white border-slate-300 text-slate-600' : 'bg-green-50 border-green-200 text-green-700'
                          }`}>{t.status}</span>
                      </td>
