...
export default function EscalationRules() {
  ...
  const [newRule, setNewRule] = useState({ stage: "Underwriting", hours: "", action: "Notify Supervisor", priority: "Medium" });
  ...
  const addRule = async () => {
    ...
    try {
      const payload = {
        stage: newRule.stage,
        trigger_delay_hours: parseInt(newRule.hours), // This will be converted in service
        action: newRule.action
      };
      const createdRule = await escalationAPI.create(payload);
    
    // Log the escalation rule creation
    // await logCreate('ESCALATION', `Created escalation rule: ${newRule.stage} - ${newRule.action} after ${newRule.hours} hours`);
    ...
    if (err.response) {
      console.error("Error creating rule (Server response):", err.response.data);
      console.error("Status:", err.response.status);
    } else {
      console.error("Error creating rule:", err.message);
    }
    
    // Handle custom errors (from service) vs HTTP errors
    if (err.isCustomError || (err.message && !err.response)) {
      // Custom error thrown from service
      alert(err.message);
    } else if (err.response?.data?.non_field_errors) {
      // Backend validation errors
      const nonFieldErrors = err.response.data.non_field_errors;
      if (nonFieldErrors.some(error => 'unique' in error.toLowerCase())) {
        alert("This rule already exists. You cannot create duplicate rules with the same stage and action combination.");
      } else {
        alert(`Validation error: ${nonFieldErrors.join(', ')}`);
      }
    } else if (err.response?.status === 400) {
      alert(`Invalid data: ${JSON.stringify(err.response?.data || 'Unknown error')}`);
    } else {
