...
export default function AddBusiness() {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      const payload = BusinessService.buildBusinessPayload(form);
      ...
      if (data && typeof data === "object") {
        ...
        alert(err?.response?.data?.detail || err?.message || "Failed to add business.");
      }
      console.error("Add business error:", err?.response?.data);
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 gap-6 pl-0 md:pl-4">
              ...
              <div>
                ...
                <MultiSelectDropdown
                  options={products}
                  selectedIds={form.mapped_loan_products}
                  onChange={(val) => setForm({ ...form, mapped_loan_products: val })}
