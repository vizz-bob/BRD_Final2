...
export default function Calendar() {
  ...
  const fyCounts = useMemo(
    () => ({
      Active: (Array.isArray(financialYears) ? financialYears : []).filter((f) => f?.status === "active").length,
      Inactive: (Array.isArray(financialYears) ? financialYears : []).filter((f) => f?.status === "inactive").length,
    }),
    [financialYears]
  );
  ...
  const ayCounts = useMemo(
    () => ({
      Active: (Array.isArray(assessmentYears) ? assessmentYears : []).filter((a) => a?.status === "active").length,
      Inactive: (Array.isArray(assessmentYears) ? assessmentYears : []).filter((a) => a?.status === "inactive").length,
    }),
    [assessmentYears]
  );
  ...
  const loadFinancialYears = async () => {
    try {
      ...
      setFinancialYears(Array.isArray(res?.data) ? res.data : (res?.data?.results || []));
    } catch (err) {
  ...
  const loadReportingPeriods = async () => {
    try {
      ...
      setReportingPeriods(Array.isArray(res?.data) ? res.data : (res?.data?.results || []));
    } catch (err) {
  ...
  const loadAssessmentYears = async () => {
    try {
      ...
      setAssessmentYears(Array.isArray(res?.data) ? res.data : (res?.data?.results || []));
    } catch (err) {
  ...
  const generateAYName = (linkedFyId) => {
    ...
    const linkedFY = (Array.isArray(financialYears) ? financialYears : []).find(fy => fy.id === parseInt(linkedFyId));
    if (!linkedFY) return "";
  ...
  const saveFinancialYear = async () => {
    ...
    const hasActiveFY = (Array.isArray(financialYears) ? financialYears : []).some(fy => fy.status === "active");
              ...
              <table className="w-full text-sm border-collapse">
                ...
                <tbody>
                  {(Array.isArray(financialYears) ? financialYears : []).length === 0 ? (
                    ...
                    (Array.isArray(financialYears) ? financialYears : []).map((f) => (
          ...
          <section className="bg-white rounded-xl shadow-md border p-4">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm border-collapse">
                ...
                <tbody>
                  {(Array.isArray(assessmentYears) ? assessmentYears : []).length === 0 ? (
                    <tr>
                    ...
                    (Array.isArray(assessmentYears) ? assessmentYears : []).map((a) => (
                      <tr key={a.id} className="border-b hover:bg-gray-50">
                        ...
                        <td>
                          {(Array.isArray(financialYears) ? financialYears : []).find(fy => fy.id === a.financial_year)?.name || "Unknown"}
                        </td>
                      </tr>
                    ))
                  )}
              ...
              <table className="w-full text-sm border-collapse">
                ...
                <tbody>
                  {(Array.isArray(reportingPeriods) ? reportingPeriods : []).length === 0 ? (
                    ...
                    (Array.isArray(reportingPeriods) ? reportingPeriods : []).map((r) => (
