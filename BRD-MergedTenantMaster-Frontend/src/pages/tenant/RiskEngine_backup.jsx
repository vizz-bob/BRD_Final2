...
function RuleIdentification() {
  ...
  const [ruleType, setRuleType] = useState("knockout");
  return (
...
function ClientProfileRules() {
  const [employerTypes, setEmployerTypes] = useState(["Public", "Govt"]);
  const [ageMin, setAgeMin] = useState("21");
  const [ageMax, setAgeMax] = useState("60");
  const [coAgeMin, setCoAgeMin] = useState("18");
  const [coAgeMax, setCoAgeMax] = useState("65");
  const [businessAge, setBusinessAge] = useState("2");
  const [addressCriteria, setAddressCriteria] = useState("match");
  const [negativeAreas, setNegativeAreas] = useState(["110059", "400063"]);
  const [areaInput, setAreaInput] = useState("");
  const sectors = ["Manufacturing", "IT/ITES", "Retail", "Healthcare", "Real Estate", "FMCG"];
  const [selectedSectors, setSelectedSectors] = useState(["IT/ITES", "Healthcare"]);
...
function FinancialCriteria() {
  const [itrToggle, setItrToggle] = useState(true);
  const [complianceToggle, setComplianceToggle] = useState(false);
  const [cashFlowToggle, setCashFlowToggle] = useState(true);

  return (
    <div className="mt-4 space-y-5">
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        <div><Label>Min Monthly Salary (₹)</Label><Input type="number" placeholder="25000" /></div>
        <div><Label>Min Business Income (₹)</Label><Input type="number" placeholder="50000" /></div>
        <div><Label>Min Annual Turnover (₹)</Label><Input type="number" placeholder="600000" /></div>
        <div><Label>Min Bank Balance (₹)</Label><Input type="number" placeholder="10000" /></div>
        <div><Label>Cash Flow Threshold (₹)</Label><Input type="number" placeholder="15000" /></div>
        <div><Label>FOIR Limit (%)</Label><Input type="number" placeholder="55" /></div>
      </div>
    ...
    <div className="mt-4 space-y-5">
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div>
          ...
          <Select><option>Mandatory</option><option>Optional</option><option>Not Required</option></Select>
        </div>
        <div>
          <Label>Ownership Verification</Label>
          <Select><option>Self-Owned</option><option>Co-Owned</option><option>Any</option></Select>
        </div>
        <div><Label>Min Estimated Value (₹)</Label><Input type="number" placeholder="500000" /></div>
        <div><Label>Max LTV Ratio (%)</Label><Input type="number" placeholder="75" /></div>
      </div>
    ...
    <div className="mt-4 space-y-5">
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {["Professional", "Salaried Employee", "Corporate Group"].map((persona) => (
            <div key={persona} className="border border-slate-200 rounded-xl p-3 bg-slate-50">
              <p className="text-xs font-bold text-slate-700 mb-2">{persona}</p>
              <div className="space-y-2">
                <div><Label>Min Internal Score</Label><Input type="number" placeholder="60" /></div>
                <div><Label>Weight (%)</Label><Input type="number" placeholder="30" /></div>
              </div>
            </div>
          ))}
        </div>
...
function VerificationRules() {
  const [mismatchFlag, setMismatchFlag] = useState(true);
  const [dupPan, setDupPan] = useState(true);
  const [internalVerif, setInternalVerif] = useState(true);
  const [agencyVerif, setAgencyVerif] = useState(false);
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="border border-slate-200 rounded-xl p-4 bg-slate-50">
          ...
          <div className="space-y-2">
            ...
            <div><Label>Turnaround Time (Days)</Label><Input type="number" placeholder="2" /></div>
        ...
        <div className="border border-slate-200 rounded-xl p-4 bg-slate-50">
          ...
          <div className="space-y-2">
            <div><Label>Agency Name</Label><Input placeholder="e.g. Karza, IDfy" /></div>
