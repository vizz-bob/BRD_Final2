...
import { FiPlus, FiSearch, FiChevronDown, FiArrowDown, FiAlignLeft, FiArrowLeft } from "react-icons/fi";
  ...
  return (
    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-6">
      ...
      {showAction && actionLabel && (
        <Button label={actionLabel} icon={actionIcon} onClick={onAction} variant="primary" />
      )}
    </div>
  );
};
...
// ✅ UPDATED — now supports required red star
...
export const MultiSelectField = ({ label, values = [], onChange, options = [] }) => {
...
export function ListView({ data = [], columns = [], actions = [], rowKey = "uuid" }) {
  return (
          ...
          >
            ...
            <div className="flex justify-end gap-2">
              {actions.map((action, i) => (
                <IconButton key={i} color={action.color || "gray"} onClick={() => action.onClick(row)}>
                  {action.icon}
                </IconButton>
              ))}
            </div>
            ...
            <div className="px-4 py-3 space-y-3 text-sm">
              {columns.slice(1).map((col) => (
                <MobileRow
                  key={col.key}
                  label={col.label}
                  value={col.render ? col.render(row[col.key], row)
                    : col.type === "status" ? <StatusBadge status={row[col.key]} />
                    : row[col.key]}
                />
              ))}
...
export const IconButton = ({ children, color, onClick }) => {
  ...
  return (
    <button onClick={onClick} className={`p-2 rounded-full transition ${map[color]}`}>
      {children}
    </button>
  );
};
            ...
            {open && (
              <div className="absolute right-0 mt-2 w-36 bg-white rounded-xl shadow-lg z-20 overflow-hidden">
                <button
                  ...
                  className={`w-full px-4 py-2 text-left text-sm hover:bg-gray-100 ${!filter ? "bg-gray-100 font-medium" : ""}`}
                >
                  All
                </button>
                {filters.map((f) => (
                  <button
                    key={f.value ?? f}
                    onClick={() => { onFilterChange(f.value ?? f); setOpen(false); }}
                    className={`w-full px-4 py-2 text-left text-sm hover:bg-gray-100 ${filter === (f.value ?? f) ? "bg-gray-100 font-medium" : ""}`}
                  >
                    {f.label ?? f}
                  </button>
                ))}
...
export const Header = SubPageHeader;
export const Select = SelectField;

export const SaveBtn = ({ label = "Save", onClick, ...props }) => (
  <Button variant="primary" onClick={onClick} {...props}>{label}</Button>
);
