...
export default function GeoFencingMap() {
  ...
  const fetchZones = async () => {
    ...
    try {
      ...
      setBlockedZones(Array.isArray(res.data) ? res.data : (res.data?.results || []));
    } catch (e) {
      ...
      <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50">
        ...
        <div className="flex gap-2">
          <span className="text-[10px] font-bold bg-rose-100 text-rose-700 px-2 py-1 rounded border border-rose-200">
            {(Array.isArray(blockedZones) ? blockedZones : []).length} Active Zones
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3">
        ...
        <div className="col-span-2 p-6 bg-slate-50/50">
          <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2">
            {(Array.isArray(blockedZones) ? blockedZones : []).map(zone => (
            ...
            {(Array.isArray(blockedZones) ? blockedZones : []).length === 0 && (
