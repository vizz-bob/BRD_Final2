...
const base_url = "adminpanel/approval-master";
...
export const approvalMasterService = {
  ...
  toggleStatus: async (id) => {
    const current = await axiosInstance.get(`${base_url}/approval-list/${id}/`);
    const nextStatus = current?.data?.status === "ACTIVE" ? "INACTIVE" : "ACTIVE";
    return axiosInstance.patch(`${base_url}/approval-list/${id}/`, { status: nextStatus });
  }
};
