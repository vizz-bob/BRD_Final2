...
class TenantRulesView(APIView):
    ...
    def get(self, request, tenant_id=None):
        ...
        # TENANT RULES
        # -----------------------------
        rule = TenantRule.objects.filter(tenant__tenant_id=tenant_id).first()
        if not rule:
            return Response({}, status=status.HTTP_200_OK)
    ...
    def post(self, request, tenant_id=None):
        # RBAC Check
        if not request.user.has_rbac_permission("WORKFLOW_SETUP"):
            return Response(
                {"detail": "You do not have permission to modify rules."},
