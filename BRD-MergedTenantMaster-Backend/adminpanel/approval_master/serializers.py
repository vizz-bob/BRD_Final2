...
class EscalationMasterSerializer(serializers.ModelSerializer):
    ...
    # Explicitly handle ForeignKey relationships
    escalation_manager = serializers.PrimaryKeyRelatedField(queryset=User.objects.all())
    escalation_to = serializers.PrimaryKeyRelatedField(queryset=User.objects.all())
    ...
    def validate(self, data):
        ...
        if not data.get("escalation_level"):
            raise serializers.ValidationError(
                {"escalation_level": "Escalation level is required"}
            )
        if not data.get("escalation_time"):
            raise serializers.ValidationError(
                {"escalation_time": "Escalation time is required"}
            )
        if not data.get("escalation_manager"):
            raise serializers.ValidationError(
                {"escalation_manager": "Escalation manager is required"}
            )
        if not data.get("escalation_to"):
            raise serializers.ValidationError(
                {"escalation_to": "Escalation to is required"}
            )
        return data
